...
ENEMY_IDLE_ANIMATION = {"fps": 0.45, "frames": [1, 2]}
...
class PunchBagEnemy:
    ...
    def __init__(self, image_manager: ImageManager, position_x: float, position_y: float):
        ...
        # Visuals
        self.bitmap, self.palette = image_manager.get_image(ENEMY_SPRITE)

        self.character_sprite = displayio.TileGrid(
            bitmap=self.bitmap,
            pixel_shader=self.palette,
            tile_width=ENEMY_SPRITE_TILE_SIZE["width"],
            tile_height=ENEMY_SPRITE_TILE_SIZE["height"],
        )
