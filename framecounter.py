...
class FrameCounter:
    ...
    def __init__(self):
        ...
        # Total frames
        self.total_frames_label = label.Label(terminalio.FONT, text=str(self.total_frames), color=0xFF0000)
        self.sprite.append(self.total_frames_label)
        
        # FPS
        self.start_time = time.monotonic()
        self.frames_per_second_label = label.Label(terminalio.FONT, y=10, text=str(int(self.frames_per_second)), color=0xFF0000)
    ...
    def loop(self):
        ...
        if (current_time - self.start_time) > FRAMES_PER_SECOND_UPDATE_SPEED:
            self.frames_per_second = self.frames_per_second_counter / (current_time - self.start_time)
